...
def emit_c_array():
    """Emit sorted unique offsets (excluding nosys) as a C array for v10d."""
    all_entries = batch0 + batch1 + batch2
    unique_offsets = sorted(set(off for _, off in all_entries))
    # Remove nosys
    unique_offsets = [off for off in unique_offsets if off != NOSYS_OFFSET]

    print(f"/* {len(unique_offsets)} unique sysent fn ptr offsets (excluding nosys) */")
    print(f"/* Generated by dedup_sysent.py */")
    print(f"#define NUM_UNIQUE {len(unique_offsets)}")
    print(f"#define BATCH_PROBE_SIZE 50")
    print(f"static const uint32_t unique_offsets[{len(unique_offsets)}] = {{")

    for i in range(0, len(unique_offsets), 8):
        chunk = unique_offsets[i:i+8]
        line = ", ".join(f"0x{off:06X}" for off in chunk)
        comma = "," if i + 8 < len(unique_offsets) else ""
        print(f"    {line}{comma}")
